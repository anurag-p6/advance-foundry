...
contract DeployMoodNftTest is Test {
  ...
  string memory convertedImageURI = deployMoodNft.svgToImageURI(svg);
  console.log(expectedURI);
  console.log(convertedImageURI);
  
  assert(keccak256(abi.encodePacked(expectedURI)) == keccak256(abi.encodePacked(convertedImageURI)));
